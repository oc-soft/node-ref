...
exports.refType = function refType (type, external) {
  ...
  if (_type.name) {
    ...
    if (external) {
      Object.defineProperty(rtn, 'external', {
...
exports.coerceType = function coerceType (type) {
  ...
  if (typeof rtn === 'string') {
    ...
    } else if (rtn === 'external') {
      rtn = exports.refType(exports.types.void, true) // void *
    } else if (rtn === 'string') {
...
exports.get = function get (buffer, offset, type) {
  ...
  let srcType
  if (type) {
    srcType = exports.coerceType(type)
  } else {
    srcType = exports.getType(buffer)
  }
  debug('get(): (offset: %d)', offset, buffer)
  assert(srcType.indirection > 0, '"indirection" level must be at least 1')
  if (srcType.indirection === 1) {
    // need to check "type"
    return srcType.get(buffer, offset)
  } else {
    // need to create a deref'd Buffer
    let size = exports.sizeof.pointer
    let destExternal = false
    if (type) {
        destExternal = type.external ?? false
    }
    
    if (srcType.indirection === 2) { 
       size = srcType.size 
    }
    let reference
    if (srcType.indirection !== 2 || !srcType.external || offset) {
      reference = exports.readPointer(buffer, offset, size, destExternal)
    } else {
      reference = exports.readFromPointer(buffer, offset, size)
    }
    if (type) {
      reference.type = exports.derefType(type)
    } else {
      reference.type = exports.derefType(srcType)
    }
    return reference
  }
}
...
exports.set = function set (buffer, offset, value, type) {
  ...
  if (type.indirection === 1) {
    ...
    const valueTypeExternal = type.external ?? false
    exports.writePointer(buffer, offset, value, valueTypeExternal)
  }
}
...
exports.writePointer = function writePointer (buf, offset, ptr, external) {
  debug('writing pointer to buffer', buf, offset, ptr, external)
  exports._writePointer(buf, offset, ptr, external)
 ...
 */
exports.readFromPointer = function(pointerBuffer, offset, size) {
  const type = exports.getType(pointerBuffer)
  let result
  if (type.indirection == 2) {
    result = Buffer.alloc(size)
    const dstContainer = Buffer.alloc(exports.sizeof.pointer)
    dstContainer.writePointer(result)
    exports.addOffset(pointerBuffer, offset)
    exports.copyMemory(dstContainer, pointerBuffer, size) 
    exports.addOffset(pointerBuffer, -offset) 
  }
  return result
}
 ...
 */
exports.comparePointer = function(
  pointerBuffer1,
  pointerBuffer2,
  offset1, offset2) {
  let result = 0;
  ...
  for (i = 0; i < exports.sizeof.pointer; i++) {
    result = pointerBuffer1[i + offset1] - pointerBuffer2[i + offset2]      
    if (result) {
      break
    }
  }
...
types.CString = {
  ...
  , indirection: 1
  , get: function get (buf, offset) {
      var _buf = exports.readPointer(buf, offset, exports.sizeof.pointer)
...
exports.getNullPointer = function(external) {
  let result = undefined
  if (external) {
    result = Buffer.alloc(exports.sizeof.pointer, 0)
    result.type = exports.coerceType('external')
  } else {
    result = exports.NULL_POINTER
  }
  return result
}

/**
 * you get true if buffer has null pointer at offset.
 * @param {Uint8Array} buf buffer
 * @param {int?} offset
 */
exports.containsNullPointer = function(buf, offset) {
  const offsetValue = offset ? offset : 0
  const nullPtrBuf = exports.getNullPointer(true)

  return exports.comparePointer(buf, nullPtrBuf, offsetValue, 0) == 0
...
Buffer.prototype.address = function address (external) {
  return exports.address(this, 0, external)
}
...
Buffer.prototype.hexAddress = function hexAddress (external) {
  return exports.hexAddress(this, 0, external)
}
...
Buffer.prototype.readPointer = function readPointer (offset, size, external) {
  if (external === void 0 && typeof size === 'boolean') {
    external = size
    size = exports.sizeof.pointer
  }
  if (size === void 0) {
  ...
  return exports.readPointer(this, offset, size, external)
}
...
Buffer.prototype.writePointer = function writePointer (ptr, offset, external) {
  if (external === void 0 && typeof offset === 'boolean') {
    external = offset
    offset = 0
  }
  return exports.writePointer(this, offset, ptr, external)
}
